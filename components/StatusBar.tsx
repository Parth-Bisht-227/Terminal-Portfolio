...
import { User, MapPin, Target, Activity, ShieldCheck, Sun, Moon } from 'lucide-react';
...
const StatusBar: React.FC<StatusBarProps> = ({ theme, toggleTheme }) => {
  ...
  const StatusItem = ({ icon: Icon, label, value, lightColor, darkColor }: any) => (
    <div className="flex items-center gap-2 px-4 border-r border-black/5 dark:border-white/10 h-full">
  ...
  return (
    <header className="fixed top-0 left-0 right-0 h-10 border-b border-black/5 dark:border-white/10 glass z-[100] flex items-center justify-between px-2 overflow-x-auto whitespace-nowrap scrollbar-hide">
      <div className="flex items-center h-full">
        ...
        <StatusItem icon={MapPin} label="Location" value={SYSTEM_DATA.location} lightColor="#16a34a" darkColor={COLORS.GREEN} />
        <StatusItem icon={Target} label="Target" value={SYSTEM_DATA.target} lightColor="#7c3aed" darkColor={COLORS.PURPLE} />
        <StatusItem icon={ShieldCheck} label="Health" value={SYSTEM_DATA.health} lightColor="#d97706" darkColor={COLORS.AMBER} />
      </div>
